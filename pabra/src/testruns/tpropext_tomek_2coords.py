...
class Setup(object):
    def __init__(self):
        ...
        args1 = {'log_int':300.0, 
                 ...
                 'coord_funs':(lambda t,x: 1.0*x[0]/x[4], lambda t,x: 1.0*max(x[1], x[3])/x[4], ),
                 'interfaces': ( np.concatenate( (np.arange(.14, .30, .04), np.arange(.30, .66, .03) )), np.concatenate( (np.arange(.14, .30, .04), np.arange(.30, .66, .03) )), ), 
                 'n_max':5,
                 'size_limits':(1., 32.),
...
def set_variables(base, pars, totaltime, n_traj, bintime, comment):
    ...
    testdir     = base + 'test_2coords_' + pars + '_' + totaltime + 's' + comment +'/'
