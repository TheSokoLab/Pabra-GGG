...
void random_var::rnd_add(const double vl,const double wght)
{
	...
	//for (int i = 0; i < max_moments; i++) moments[i] += wght*pow(vl,i+1); // old, unefficient version
	
	for (int i = 0; i < max_moments; i++)	// corrected version
	{
		summand *= vl;
		moments[i] += summand;
	}
...
double random_var::variance(void)
{
	...
	return sqrt(av_sq - av*av);	// ATTENTION! This is the standard deviation, not the variance, if one is strict...
}
