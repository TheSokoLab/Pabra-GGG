...
class PathExtractor(object):
	...
	self.paths_used = self.paths # choose here which of the sets to output (self.paths or self.paths_longest)
        ...
        for pid, path_with_numbers in self.paths_used: # in self.paths or self.paths_longest
              ...
              if have_snapshot_at(t):
                  ...
                  for x in xvector:
                      ...
                      # Concentration data starts at index self.conc_start
                          ...
                          # in reverse order, i.e. starting from gene A
                          for j in [0, 1, 2, 3] :
    ...
    def create_gnuplot_file(self, plot):
        ...
        plots = ''
        for pid, path in self.paths_used:
	...
	convcommand = '!ps2pdf ' + plot.name + '.ps\n'
	fout.write(convcommand)
...
class Basin:
	...
	print("Basin timebins = " + str(self.timebins))
...
class TrunkExtractor(object):
    ...
    def __init__(self, filename, nPSbins, nTbins, psmax, histtime, rc_projection_mode='default', extra_tag=None):
        ...
        self.rc_projection_mode = rc_projection_mode
          # set to 'default', 'promoters', 'A-C' or 'B-D' (see below for more detail)
        self.rc_velocities_min_t = 0.0   # minimial time from which on trajectories are
        ...
        self.basin_numbers = [0, 1, 2, 3, 4, 5, 6, 7, 8]
        ...
        self.basin.append( Basin(rectangular=True, lo=0.00, hi=0.05, lt=0.40, rt=1.00, timebins=self.nTbins+1) ) # A-C coordinates: C destroyed
        self.basin.append( Basin(rectangular=True, lo=0.40, hi=1.00, lt=0.00, rt=0.05, timebins=self.nTbins+1) ) # A-C coordinates: A destroyed
        self.basin.append( Basin(rectangular=True, lo=0.00, hi=0.10, lt=0.40, rt=1.00, timebins=self.nTbins+1) ) # same, but somewhat larger...
        self.basin.append( Basin(rectangular=True, lo=0.40, hi=1.00, lt=0.00, rt=0.10, timebins=self.nTbins+1) )
    ...
    def process_reaction_coordinates(self, rclist, mode='default'):
        ...
        assert mode in ['default', 'promoters', 'A-C', 'B-D', 'differences']
        ...
        # of the two antagonistic genes
            ...
            rc_maxAC = max(rc_A, rc_C)
            rc_maxBD = max(rc_B, rc_D)
            rc_total = rclist[4]
        ...
        rc_lambda = (rc_maxAC + rc_maxBD) / rc_total
        ...
        # in phase space (both for the standard and the promoter based case)
            rc_psX = rc_maxAC / rc_total
            rc_psY = rc_maxBD / rc_total

        elif mode == 'A-C':
        # Plotting [C] vs. [A] in phase space (normalized by total protein)
            rc_psX = rc_A / rc_total
            rc_psY = rc_C / rc_total

        elif mode == 'B-D':
        # Plotting [D] vs. [B] in phase space (normalized by total protein)

            rc_psX = rc_B / rc_total
            rc_psY = rc_D / rc_total

        elif mode == 'differences':
        # Plotting (normed) [A]-[C] vs [B]-[D]
        # Values should be between 0 and 1 and positive, so we shift a bit
        # and renormalize

            rc_psX = 0.5 + 0.5 * (rc_A - rc_C) / rc_total
            rc_psY = 0.5 + 0.5 * (rc_B - rc_D) / rc_total
    ...
    def accumulate_data(self, histtime):
        ...
        for twt in self.trunks:
		    ...
		    ###print "# have_previous_rc_values = " + str(have_previous_rc_values) + "\n"
	...
	### DEBUG INFO
	print "# Info: length of displacements list = " + str(len(self.displacements)) + "\n"
	print "# Info: length of phase space histogram list = " + str(len(list_H_ps)) + "\n"
    ...
    def write_one_histogram(self, H, E, nbins, name, comment=''):
        ...
        # Assert that H and E have the right format,
    ...
    def write_histograms(self):
        ...
        for ts in self.t_ps_snapsh:
            ...
            s = s+1
	    ### HACK: Currently switched off because it fails for some trees... (Tomek)
    ...
    def write_displacement_statistics(self):
        ...
        for li in [0,1,2]:
            ...
            for xi in range(self.nPSbins):
		      ...
		      wt      = self.A_ps_wt[li][xi][yi]

                      fout.write('%s %s %s %s %s %s %s %s %s %s %s\n' \
                                  % (this_x, this_y, avg_dX, avg_dY, avg_dR2, avg_Jx, avg_Jy, cnt, avg_cX, avg_cY, wt) )
    ...
    def write_running_averages(self):
        ...
        # Reminder:
        # psX = phasespace X coord. = typically either [A] or max([A],[C])
        # psY = phasespace Y coord. = typically max([B],[D])
    ...
    def create_gnuplot_file(self, plot):
        ...
        lstyle = " w l lw 5 "
        # TODO The if statement here is not the most elegant solution (but works)...
...
class AutocorrelationCalculator(object):
    ...
    def iterate(self):
        ...
        # TODO Maybe a different iteration type is better?
...
if __name__ == '__main__':
    ...
    if str(taskname) == "histograms" :
        ...
        for mode in [ 'default']: #, 'A-C', 'B-D', 'differences' ]: # Possible options: 'default', 'A-C', 'B-D', 'differences', 'promoters'
            ...
            if mode=='A-C':
                ...
                p.xlabel = "0.5 + 0.5 * ([A]-[C])/[all]"
                p.ylabel = "0.5 + 0.5 * ([B]-[D])/[all]"
            else: # 'default' or 'promoters'
                p.xlabel = "max([A],[C])/[all]"
                p.ylabel = "max([B],[D])/[all]"
            p.xrange = "[0.2:0.7]"
            p.yrange = "[0.1:0.6]"
